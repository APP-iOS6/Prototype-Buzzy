    ...
    public func increaseLike(for postID: UUID) {
        ...
        public func toggleScrap(postID: UUID) {
            if let index = posts.firstIndex(where: { $0.id == postID }) {
                ...
                qaPosts[index].isScrapped.toggle()
            }
        }
        
        public func addReply(to postID: UUID, commentID: UUID, author: String, content: String) {
            if let postIndex = posts.firstIndex(where: { $0.id == postID }),
               let commentIndex = posts[postIndex].comments.firstIndex(where: { $0.id == commentID }) {
                let newReply = BoardComment(author: author, content: content)
                posts[postIndex].comments[commentIndex].replies.append(newReply)
            } else if let postIndex = qaPosts.firstIndex(where: { $0.id == postID }),
                      let commentIndex = qaPosts[postIndex].comments.firstIndex(where: { $0.id == commentID }) {
                let newReply = BoardComment(author: author, content: content)
                qaPosts[postIndex].comments[commentIndex].replies.append(newReply)
            }
        }
    }
